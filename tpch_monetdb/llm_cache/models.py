...
MODEL_REGISTRY: dict[str, ModelPricing] = {
    ...
    # DeepSeek V4 series (added for assignment)
...
def request_cost_usd(
    ...
    For DeepSeek models, uses separate cached/uncached rates.
    ...
    Returns:
        Total cost in USD
    """
    # DeepSeek handling (TODO completed)
    model_lower = str(model).lower()
    if "deepseek-v4" in model_lower:
        # Normalize model name to get base key
        normalized = normalize_accounting_model_name(str(model))
        # Try to get pricing; fallback to prefix match
        try:
            pricing = get_model_pricing(normalized)
        except KeyError:
            # Fallback: try "deepseek-v4-flash" or "deepseek-v4-pro"
            if "flash" in model_lower:
                pricing = get_model_pricing("deepseek-v4-flash")
            else:
                pricing = get_model_pricing("deepseek-v4-pro")
        # Ensure cached tokens don't exceed input
        actual_cached = min(cached_tokens, input_tokens)
        uncached = input_tokens - actual_cached
        return (
            uncached * pricing.input
            + actual_cached * pricing.cached_input
