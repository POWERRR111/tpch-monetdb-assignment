...
class StageToolRuntime:
    ...
    def list_directory(self, relative_path: Optional[str], pattern: Optional[str], limit: int) -> str:
        ...
        profile = self.require_tool("list_files")
        ...
        if not self._is_under_extra_read_root(target):
            try:
                target.relative_to(workspace)
            except ValueError:
                # 使用 _recoverable_error 抛出可恢复错误
                raise self._recoverable_error(
                    code="PATH_OUTSIDE_WORKSPACE",
                    category="stage_policy",
                    message=f"Path {target} is outside workspace",
                    relevant_files=self._profile_scope_files(),
                    allowed_next_actions=profile.read_globs,
                    recommended_next_action="Use a path within the workspace.",
                )
    ...
    def grep_repo(self, pattern: str, relative_path: Optional[str], glob: Optional[str], limit: int) -> str:
        ...
        profile = self.require_tool("grep_repo")
        workspace = self.workspace_root.resolve()
        if relative_path in (None, "", ".", "/"):
            root = workspace
        else:
            root = (workspace / relative_path).resolve()
        if not self._is_under_extra_read_root(root):
            try:
                ...
                raise self._recoverable_error(
                    code="PATH_OUTSIDE_WORKSPACE",
                    category="stage_policy",
                    message=f"Path {root} is outside workspace",
                    relevant_files=self._profile_scope_files(),
                    allowed_next_actions=profile.read_globs,
                    recommended_next_action="Use a path within the workspace.",
                )
        ...
        if skipped_large > 0:
            result += f"\ngrep_repo skipped {skipped_large} large file" + ("s" if skipped_large > 1 else "")
        return result
