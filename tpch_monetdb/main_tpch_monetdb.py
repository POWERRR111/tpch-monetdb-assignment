...
def _normalize_deepseek_reasoning_effort(effort: Optional[str]) -> str:
    if effort is None:
        return "high"
    ...
    if effort_lower == "none":
        return "disabled"
    return "high"   # minimal, low, medium, high 都映射为 high
...
def _build_model_settings(
    ...
    raw_reasoning = _resolve_reasoning(args, model_config)
    ...
    if model_config.use_litellm:
        if model_is_deepseek:
            # 根据 reasoning 是否为空决定 allowed_openai_params
            effort = getattr(raw_reasoning, "effort", None)
            if effort == "none" or raw_reasoning is None:
                # 显式禁用 reasoning
                extra_args = {
    ...
    if model_is_deepseek:
        if raw_reasoning is None:
            ...
            effort = getattr(raw_reasoning, "effort", None)
            normalized_effort = _normalize_deepseek_reasoning_effort(effort)
